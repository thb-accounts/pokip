...
const fail = (c: string, m: string): never => {
  throw new HttpsError(c as never, m);
};
async function profile(uid: string) {
  const s = await db.doc(`users/${uid}`).get();
  if (!s.exists) fail("not-found", "Account profile not found.");
  return s.data()!;
}
export const awardPoints = onCall(async (r) => {
  ...
  const { customerUid, merchantId, purchaseAmount, reference, note } =
    r.data ?? {};
  if (
  ...
  await db.runTransaction(async (t) => {
    ...
    if (reference) {
      ...
      if (!d.empty) fail("already-exists", "Receipt already awarded.");
    }
    const points = Math.floor(
      purchaseAmount * (m.pointsRule?.pointsPerCurrencyUnit || 1),
    );
    if (points < 1) fail("invalid-argument", "Purchase does not earn points.");
    const b = db.doc(`pointsBalances/${merchantId}_${customerUid}`),
      old = (await t.get(b)).data()?.balance || 0,
      newBalance = old + points,
      tx = db.collection("pointsTransactions").doc();
    t.set(
      b,
      {
        merchantId,
...
export const redeemItem = onCall(async (r) => {
  const uid = r.auth?.uid;
  if (!uid) fail("unauthenticated", "Sign in required.");
  ...
  return db.runTransaction(async (t) => {
    const id = `redemptionRequests/${uid}_${requestId}`,
      dup = await t.get(db.doc(id));
    ...
    const b = db.doc(`pointsBalances/${l.merchantId}_${uid}`),
      old = (await t.get(b)).data()?.balance || 0;
    if (old < l.pointsPrice)
      fail("failed-precondition", "Insufficient points.");
    const red = db.collection("redemptions").doc(),
      code = red.id.slice(0, 8).toUpperCase();
    t.set(
      b,
      {
